...
llm = HuggingFaceHub(repo_id="rinna/japanese-gpt2-medium", model_kwargs={"temperature":config['temperature']})
...
llm(f"ユーザー: あなたはDatabricksによって開発された有能なアシスタントであり、指定された文脈に基づいて質問に回答することを得意としており、文脈はドキュメントです。文脈からよい回答が得られた場合には、質問に回答するために文脈を要約してください。<NL>システム: 質問は何ですか？<NL>ユーザー: {question}<NL>システム: 文脈は何ですか？<NL>ユーザー: {docs[0].page_content}<NL>システム: ")
