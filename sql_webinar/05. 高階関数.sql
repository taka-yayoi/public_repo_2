...
-- MAGIC %md ## シンプルな例
-- MAGIC 
-- MAGIC 基本的な変換処理でコンセプトの基礎を学びましょう。このケースでは、高階関数``transform``が配列``values``に対してイテレーションを行い、関連づけられたラムダ関数をそれぞれの要素に適用し、新たな配列を作成します。ラムダ関数``element + 1``は、それぞれの要素をどの様に操作するのかを指定します。このSQLは以下の様になります。
...
-- MAGIC </p>
-- MAGIC 
-- MAGIC 1. `TRANSFORM(values..)`は高階関数です。これは、入力として配列と匿名関数を受け取ります。内部的には、新規配列のセットアップ、それぞれの要素への匿名関数の適用、出力配列への結果の割り当てを行います。
...
-- MAGIC 
-- MAGIC これは、2つのラムダ関数を使う唯一の高階関数です。
-- MAGIC 
